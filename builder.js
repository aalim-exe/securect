...
function generateChromiumLauncher(exePaths, flag, browserName, extJsonB64, extNames, browser) {
    const psCode = [
        ...
        '    # 1. Registry - HKLM (system-wide install)',
        '    $regPaths = @(',
        ...
        '        try { $reg = Get-ItemProperty -Path $rp -ErrorAction Stop; if ($reg."(default)" -and (Test-Path $reg."(default)")) { return $reg."(default)" } } catch {}',
        '    }',
        '    # 2. Registry - HKCU (per-user install)',
        '    $regPaths = @(',
        '        "HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\$name",',
        '        "HKCU:\\SOFTWARE\\WOW6432Node\\Microsoft\\Windows\\CurrentVersion\\App Paths\\$name"',
        '    )',
        '    foreach ($rp in $regPaths) {',
        '        try { $reg = Get-ItemProperty -Path $rp -ErrorAction Stop; if ($reg."(default)" -and (Test-Path $reg."(default)")) { return $reg."(default)" } } catch {}',
        '    }',
        '    # 3. PATH via Get-Command',
        '    try { $cmd = Get-Command $name -ErrorAction Stop; if ($cmd.Source -and (Test-Path $cmd.Source)) { return $cmd.Source } } catch {}',
        '    # 4. PATH via where.exe (cmd built-in, searches all PATH entries)',
        '    try { $where = where.exe $name 2>$null; if ($where -and (Test-Path $where[0])) { return $where[0] } } catch {}',
        '    # 5. Static known paths',
        '    $staticPaths = @(' + exePaths.map(p => '"' + p.replace(/\\/g, '\\\\') + '"').join(', ') + ')',
        '    foreach ($p in $staticPaths) { if (Test-Path $p) { return $p } }',
        '    # 6. Recursive search in common install roots (finds ANY install location)',
        '    $searchRoots = @("$env:LOCALAPPDATA\\Google", "$env:ProgramFiles\\Google", "${env:ProgramFiles(x86)}\\Google")',
...
function generateFirefoxLauncher(exePaths, flag, browserName, extJsonB64, extNames, browser) {
    const psCode = [
        ...
        '    $name = "firefox.exe"',
        '    # 1. Registry - HKLM',
        '    $regPaths = @("HKLM:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\$name", "HKLM:\\SOFTWARE\\WOW6432Node\\Microsoft\\Windows\\CurrentVersion\\App Paths\\$name")',
        '    foreach ($rp in $regPaths) { try { $reg = Get-ItemProperty -Path $rp -ErrorAction Stop; if ($reg."(default)" -and (Test-Path $reg."(default)")) { return $reg."(default)" } } catch {} }',
        '    # 2. Registry - HKCU',
        '    $regPaths = @("HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\$name", "HKCU:\\SOFTWARE\\WOW6432Node\\Microsoft\\Windows\\CurrentVersion\\App Paths\\$name")',
        '    foreach ($rp in $regPaths) { try { $reg = Get-ItemProperty -Path $rp -ErrorAction Stop; if ($reg."(default)" -and (Test-Path $reg."(default)")) { return $reg."(default)" } } catch {} }',
        '    # 3. PATH via Get-Command',
        '    try { $cmd = Get-Command $name -ErrorAction Stop; if ($cmd.Source -and (Test-Path $cmd.Source)) { return $cmd.Source } } catch {}',
        '    # 4. PATH via where.exe',
        '    try { $where = where.exe $name 2>$null; if ($where -and (Test-Path $where[0])) { return $where[0] } } catch {}',
        '    # 5. Static known paths',
        '    $paths = @(' + exePaths.map(p => '"' + p.replace(/\\/g, '\\\\') + '"').join(', ') + ')',
        '    foreach ($p in $paths) { if (Test-Path $p) { return $p } }',
        '    # 6. Recursive search',
        '    $searchRoots = @("$env:ProgramFiles\\Mozilla Firefox", "${env:ProgramFiles(x86)}\\Mozilla Firefox")',
        '    foreach ($root in $searchRoots) {',
        '        if (Test-Path $root) {',
        '            $found = Get-ChildItem -Path $root -Recurse -Filter $name -ErrorAction SilentlyContinue | Select-Object -First 1',
        '            if ($found) { return $found.FullName }',
...
function generateOperaLauncher(exePaths, flag, browserName, extJsonB64, extNames, browser) {
    ...
    const psCode = [
        ...
        'function Find-Browser {',
        '    # Try opera.exe first, then launcher.exe',
        '    foreach ($name in @("opera.exe", "launcher.exe")) {',
        '        # 1. Registry - HKLM',
        '        $regPaths = @("HKLM:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\$name", "HKLM:\\SOFTWARE\\WOW6432Node\\Microsoft\\Windows\\CurrentVersion\\App Paths\\$name")',
        '        foreach ($rp in $regPaths) { try { $reg = Get-ItemProperty -Path $rp -ErrorAction Stop; if ($reg."(default)" -and (Test-Path $reg."(default)")) { return $reg."(default)" } } catch {} }',
        '        # 2. Registry - HKCU',
        ...
        '    $paths = @(' + exePaths.map(p => '"' + p.replace(/\\/g, '\\\\') + '"').join(', ') + ')',
        '    foreach ($p in $paths) { if (Test-Path $p) { return $p } }',
        '    # 6. Recursive search in common install roots',
        '    $searchRoots = @("$env:LOCALAPPDATA\\Programs\\Opera", "$env:LOCALAPPDATA\\Programs\\Opera GX", "$env:ProgramFiles\\Opera", "$env:ProgramFiles\\Opera GX", "${env:ProgramFiles(x86)}\\Opera", "${env:ProgramFiles(x86)}\\Opera GX")',
        '    foreach ($root in $searchRoots) {',
        '        if (Test-Path $root) {',
        '            $found = Get-ChildItem -Path $root -Recurse -Filter "opera.exe" -ErrorAction SilentlyContinue | Select-Object -First 1',
        '            if ($found) { return $found.FullName }',
        '        }',
        '    }',
        '    return $null',
